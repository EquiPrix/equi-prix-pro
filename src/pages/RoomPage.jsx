...
export default function RoomPage() {
  ...
  const loadRoom = async () => {
    ...
    try {
      ...
      if (!rows || !rows.length) { setError('Room not found.'); setLoading(false); return; }
      setRoom(rows[0]);
  ...
  const checkMembership = async () => {
    ...
    const rows = await sbFetch('room_members?room_id=eq.' + room.id + '&user_email=eq.' + encodeURIComponent(user.email) + '&limit=1');
    if (rows && rows.length) setIsMember(true);
  };
  ...
  return (
    <div className="min-h-screen px-4 py-10" style={{ background: 'var(--ink)' }}>
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="rounded-xl overflow-hidden mb-6" style={{ border: '1px solid rgba(180,149,48,0.25)', background: '#14130e' }}>
          ...
          <div className="px-6 py-5 text-center" style={{ borderBottom: '1px solid rgba(180,149,48,0.15)' }}>
            ...
            {room.is_sponsored && room.sponsor_logo_url && (
              <img src={room.sponsor_logo_url} alt={room.sponsor_name} style={{ height: 40, margin: '0 auto 12px', objectFit: 'contain' }} />
            )}
          ...
          <div className="px-6 py-5">
            ...
            {!user ? (
              ...
              <div className="text-center font-cormorant italic text-base" style={{ color: '#e07070' }}>
                This room is full.
              </div>
            ) : (
        ...
        {/* Member list — only visible to members or manager */}
