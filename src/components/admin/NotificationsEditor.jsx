...
const NOTIFICATION_TYPES = [
  {
    id: 'draft_open',
    label: 'Draft Open',
    description: 'Announce picks are open + lock time',
    icon: '🟢',
  },
  {
    id: 'team_results',
    label: 'Team Results + GP Draft',
    description: 'Team results posted + GP draft open + lock time',
    icon: '🏆',
  },
  {
    id: 'final_results',
    label: 'Final Results',
    description: 'Event complete + final leaderboard',
    icon: '🎯',
  },
  {
    id: 'new_event',
    label: 'New Event',
    description: 'Announce upcoming event + sponsor',
    icon: '📣',
  },
];
...
export default function NotificationsEditor() {
  ...
  const loadRecipients = async () => {
    ...
    try {
      // Get all unique emails from picks table
      const picks = await sbFetch('picks?select=access_code&order=access_code') || [];
  ...
  const send = async () => {
    if (!selectedEventId || !recipients.length) return;
    setSending(true);
    setResult(null);
    ...
    try {
      const res = await fetch('/api/send-notification', {
        ...
        body: JSON.stringify({
          type: selectedType,
          recipients,
          eventName: selectedEvent ? `${selectedEvent.city} ${selectedEvent.dates?.split(' ')[2] || ''}`.trim() : '',
  ...
  return (
    <div>
      ...
      <p className="font-cormorant text-base italic mb-6" style={{ color: 'var(--mid)' }}>
        Send event emails to all registered users.
      </p>
      ...
      <div className="flex items-center gap-2 mb-6 px-3 py-2.5 rounded-lg"
        ...
        <span className="font-cormorant text-sm" style={{ color: 'var(--cream)' }}>
          {loadingRecipients ? 'Loading…' : `${recipients.length} registered users`}
        </span>
        <button onClick={loadRecipients}
          className="ml-auto font-cinzel text-xs"
        ...
        <div className="grid grid-cols-2 gap-2">
          {NOTIFICATION_TYPES.map(t => (
            <button key={t.id} onClick={() => setSelectedType(t.id)}
              ...
              <div className="font-cinzel text-xs" style={{ color: selectedType === t.id ? 'var(--gold)' : 'var(--cream)', fontSize: 9, letterSpacing: '0.08em' }}>
                {t.label}
              </div>
              <div className="font-cormorant text-xs italic mt-0.5" style={{ color: 'var(--mid)' }}>
                {t.description}
              </div>
            </button>
          ))}
      ...
      {/* Lock time — for draft_open and team_results */}
      {(selectedType === 'draft_open' || selectedType === 'team_results') && (
        <div className="mb-4">
          <label style={labelStyle}>LOCK TIME (shown in email)</label>
          <input
            value={lockTime}
            onChange={e => setLockTime(e.target.value)}
            placeholder="e.g. Saturday June 21 at 2:00 PM CET"
            style={inputStyle}
          />
        </div>
      )}

      {/* Sponsor name — for new_event */}
      ...
      {preview && selectedEvent && (
        <div className="mb-6 rounded-lg overflow-hidden" style={{ border: '1px solid rgba(180,149,48,0.2)' }}>
          <div className="px-3 py-2" style={{ background: 'rgba(180,149,48,0.06)', borderBottom: '1px solid rgba(180,149,48,0.1)' }}>
            <span className="font-cinzel text-xs" style={{ color: 'var(--mid)', fontSize: 9 }}>
              SUBJECT: {notificationType?.icon} {selectedEvent.city} — {notificationType?.label}
            </span>
          </div>
            ...
            <div style={{ maxWidth: 360, margin: '0 auto', background: '#14130e', border: '1px solid rgba(180,149,48,0.25)', borderRadius: 6, overflow: 'hidden', fontFamily: 'Georgia,serif' }}>
              ...
              <div style={{ padding: '20px' }}>
                <p style={{ fontSize: 16, color: '#e8e0cc', margin: '0 0 10px', fontStyle: 'italic', textAlign: 'center' }}>
                  {notificationType?.id === 'draft_open' ? 'The draft is open.' :
                   notificationType?.id === 'team_results' ? 'Team results are in.' :
                   notificationType?.id === 'final_results' ? 'Final results are in.' :
                   `${selectedEvent.city} is coming.`}
                </p>
                <p style={{ fontSize: 12, color: '#b49530', lineHeight: 1.6, margin: '0 0 16px' }}>
                  {notificationType?.id === 'draft_open'
                    ? `Your picks for ${selectedEvent.city} are now open.${lockTime ? ` Picks lock: ${lockTime}.` : ''}`
                    : notificationType?.id === 'team_results'
                    ? `Team results for ${selectedEvent.city} are posted. Lock in your GP rider picks${lockTime ? ` before ${lockTime}` : ''}.`
                    : notificationType?.id === 'final_results'
                ...
                <div style={{ textAlign: 'center' }}>
                  <span style={{ display: 'inline-block', background: '#b49530', color: '#0f0e0a', padding: '10px 24px', borderRadius: 3, fontSize: 10, fontWeight: 'bold', letterSpacing: '0.12em' }}>
                    {notificationType?.id === 'draft_open' ? 'MAKE YOUR PICKS' :
                     notificationType?.id === 'team_results' ? 'VIEW RESULTS & PICK RIDERS' :
                     notificationType?.id === 'final_results' ? 'VIEW FINAL LEADERBOARD' :
                     'OPEN EQUIPRIX'}
                  </span>
                </div>
              </div>
      ...
      <button
        onClick={send}
        disabled={sending || !selectedEventId || !recipients.length}
        ...
        style={{
          ...
          letterSpacing: '0.1em',
          opacity: (!selectedEventId || !recipients.length) ? 0.4 : 1,
        }}>
        <Send size={13} />
        ...
        <div className="mt-4 px-4 py-3 rounded-lg flex items-start gap-3"
          ...
          <div>
            {result.error ? (
              ...
              <>
                <p className="font-cormorant text-sm font-semibold" style={{ color: '#4caf7d' }}>
                  ✓ {result.sent} emails sent successfully
                </p>
                {result.failed > 0 && (
                  <p className="font-cormorant text-sm" style={{ color: '#e07070' }}>
