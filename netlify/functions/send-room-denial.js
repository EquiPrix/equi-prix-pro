export default async (req, context) => {
  ...
  const { email, roomName, prize, eventName, eventFlag, joinUrl, managerName, isApprovalNotice } = body;

  if (!email || !roomName || !joinUrl) return new Response(JSON.stringify({ error: 'Missing required fields' }), {
    status: 400, headers: { 'Content-Type': 'application/json' }
  });

  // isApprovalNotice: true means this email is going to the room's OWN
  // manager (the person who requested it), telling them it's been set up
  // and giving them the link to share — not a peer invite to join someone
  // else's room. Same template, different heading/copy/subject.
  const heading = isApprovalNotice ? 'Your room is ready.' : "You've been invited.";
  const bodyText = isApprovalNotice
    ? `Your private room <strong style="color:#c8b86a;"> ${roomName}</strong> has been approved and is ready to go${eventName ? ` for the <strong style="color:#c8b86a;">${eventFlag || ''} ${eventName}</strong>` : ''}. Share the link below with your group so they can join.`
  ...
  const ctaLabel = isApprovalNotice ? 'VIEW & SHARE YOUR ROOM' : 'JOIN THE ROOM';
  const subject = isApprovalNotice
    ? `Your room "${roomName}" is approved! 🏇`
    : `You've been invited to ${roomName} on EquiPrix 🏇`;
...
<div style="background-color:#0f0e0a;padding:40px 20px;font-family:Georgia,serif;">
  <div style="max-width:480px;margin:0 auto;background-color:#14130e;border:1px solid rgba(180,149,48,0.25);border-radius:8px;overflow:hidden;">
    ...
    <div style="padding:36px 32px;">
      <p style="font-size:24px;color:#e8e0cc;margin:0 0 14px;font-style:italic;text-align:center;">${heading}</p>
      ...
      <div style="text-align:center;margin:32px 0;">
        <a href="${joinUrl}" style="display:inline-block;background-color:#b49530;color:#0f0e0a;text-decoration:none;padding:16px 40px;border-radius:4px;font-size:12px;font-weight:bold;letter-spacing:0.15em;font-family:Georgia,serif;">${ctaLabel}</a>
      </div>
      <p style="font-size:12px;color:#5a5040;line-height:1.6;margin:0;text-align:center;">
        ${isApprovalNotice ? 'Anyone with this link can join your room — share it with your group.' : "If you don't have an EquiPrix account yet, you'll be prompted to create one — it's free."}
      </p>
    </div>
